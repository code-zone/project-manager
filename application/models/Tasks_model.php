...
class Tasks_model extends Crud_model
{
    ...
    public function get_details($options = array())
    {
        ...
        if ($assigned_to) {
            ...
            $join .= "LEFT JOIN assigned_tasks ON assigned_tasks.task_assigned = $tasks_table.id";
            $where .= " AND assigned_tasks.assigned_user=$assigned_to";
        }
        ...
        FROM $tasks_table
        LEFT JOIN $projects ON $tasks_table.project_id=$projects.id $join
    ...
    public function count_my_open_tasks($user_id)
    {
        return $this->get_my_open_tasks($user_id)->row()->total;
    }

    public function get_my_open_tasks($user_id)
    {
        $tasks_table = $this->db->dbprefix('tasks');
        $sql = "SELECT COUNT($tasks_table.id) AS total
        FROM $tasks_table
        LEFT JOIN assigned_tasks ON $tasks_table .id = assigned_tasks.task_assigned
        WHERE $tasks_table.deleted=0  AND assigned_tasks.assigned_user=$user_id AND FIND_IN_SET($tasks_table.status,'to_do,in_progress')";

        return $this->db->query($sql);
    }

    public function list_my_tasks($user_id)
    {
        return $this->get_my_open_tasks($user_id)->result();
    }

    public function get_label_suggestions($project_id)
    {
        $tasks_table = $this->db->dbprefix('tasks');
    ...
    public function get_my_projects_dropdown_list($user_id = 0)
    {
    ...
    public function get_task_statistics($options = array())
    {
        $tasks_table = $this->db->dbprefix('tasks');
