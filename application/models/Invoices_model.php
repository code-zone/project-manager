...
class Invoices_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($status === "draft") {
            ...
            $where .= " AND IFNULL(payments_table.payment_received,0)>$invoice_value_calculation";
        } else if ($status === "overdue") {
            $where .= " AND  $invoices_table.status !='draft' AND $invoices_table.due_date<'$now' AND IFNULL(payments_table.payment_received,0)<$invoice_value_calculation";
        }


        $sql = "SELECT $invoices_table.*, $clients_table.currency_symbol as currency, $clients_table.company_name, $projects_table.title AS project_title,
           $invoice_value_calculation AS invoice_value, IFNULL(payments_table.payment_received,0) AS payment_received, tax_table.percentage AS tax_percentage, tax_table2.percentage AS tax_percentage2
        FROM $invoices_table
    ...
    function get_invoice_total_summary($invoice_id = 0) {
        ...
        $item_sql = "SELECT SUM($invoice_items_table.total) AS invoice_subtotal, (SELECT $clients_table.currency_symbol FROM $clients_table WHERE $clients_table.id=$invoices_table.client_id limit 1) AS currency
        ...
        $result->balance_due = $result->invoice_total - $payment->total_paid;
        $result->currency = $item->currency;
        return $result;
    }

    function delete($id = 0, $undo = false) {
        if (parent::delete($id, $undo)) {
            $deleted_value = 1;
            if ($undo) {
                $deleted_value = 0;
            }
            //delete the payments
            $invoice_payments_table = $this->db->dbprefix('invoice_payments');
            $sql = "UPDATE $invoice_payments_table SET deleted=$deleted_value
                    WHERE $invoice_payments_table.invoice_id=$id";
            return $this->db->query($sql);
        }
    }
    ...
    function invoice_statistics() {
        ...
        $payments = "SELECT SUM($invoice_payments_table.amount) AS total, MONTH($invoice_payments_table.payment_date) AS month
            FROM $invoice_payments_table
            WHERE $invoice_payments_table.deleted=0 AND YEAR($invoice_payments_table.payment_date)=$year
        ...
        $invoices = "SELECT SUM(total) AS total, MONTH(due_date) AS month FROM (SELECT $invoice_value_calculation AS total ,$invoices_table.due_date
            ...
            LEFT JOIN (SELECT invoice_id, SUM(total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id = $invoices_table.id 
            WHERE $invoices_table.deleted=0 AND YEAR($invoices_table.due_date)=$year) as details_table GROUP BY  MONTH(due_date)";
