...
class Invoices extends Pre_loader {
    ...
    function modal_form() {
        ...
        $view_data['model_info'] = $this->Invoices_model->get_one($this->input->post('id'));
    ...
    private function _make_row($data) {
        $invoice_url = "";
        if ($this->login_user->user_type == "staff") {
            $invoice_url = anchor(get_uri("invoices/view/" . $data->id), $data->id);
        } else {
            $invoice_url = anchor(get_uri("invoices/view_pdf/" . encode_id($data->id, "invoice_id")), $data->id, array("target" => "_blank"));
        }
        ...
        return array($invoice_url,
            ...
            to_currency($data->invoice_value, $data->currency),
            to_currency($data->payment_received, $data->currency),
    ...
    private function _get_invoice_status_label($data) {
        $invoice_status_class = "label-default";
        $status = "not_paid";
        $now = get_my_local_time("Y-m-d");
        if ($data->status != "draft" && $data->due_date < $now && $data->payment_received < $data->invoice_value) {
            $invoice_status_class = "label-danger";
            $status = "overdue";
        } else if ($data->status !== "draft" && $data->payment_received <= 0) {
            $invoice_status_class = "label-warning";
            $status = "not_paid";
        } else if ($data->payment_received && $data->payment_received >= $data->invoice_value) {
            $invoice_status_class = "label-success";
            $status = "fully_paid";
        } else if ($data->payment_received > 0 && $data->payment_received < $data->invoice_value) {
    ...
    private function _make_item_row($data) {
        $item = "<b>$data->title</b>";
        if ($data->description) {
            $item.="<br /><span>" . $data->description . "</span>";
        }
        ...
        return array(
            ...
            to_currency($data->rate, $data->currency),
            to_currency($data->total, $data->currency),
    ...
    //view pdf is accessable to client also. using encoded id for extra security
    function view_pdf($invoice_id = 0) {
        $invoice_id = decode_id($invoice_id, "invoice_id");
        if ($invoice_id) {
    ...
    private function _prepare_invoice($invoice_id, $mode = "download") {
    ...
    function send_invoice_modal_form($invoice_id) {
        ...
        if ($invoice_id) {
            ...
            $parser_data["BALANCE_DUE"] = to_currency($invoice_total_summary->balance_due);
    ...
    function send_invoice() {
        ...
        if (send_app_mail($contact->email, $subject, $message, array("attachments" => array($attachement_url), "cc" => $cc))) {
            // change email status
            if ($this->Invoices_model->save(array("status" => "not_paid", "last_email_sent_date" => get_my_local_time()), $invoice_id)) {
