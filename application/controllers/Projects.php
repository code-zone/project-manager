...
class Projects extends Pre_loader
{
    ...
    public function projects_list_data_of_client($client_id)
    {
    ...
    private function _make_row($data)
    {
        ...
        } elseif ($progress !== 100 && $data->status === 'open' && get_my_local_time('Y-m-d') == $data->deadline) {
    ...
    public function view($project_id = 0)
    {
    ...
    public function overview_for_client($project_id)
    {
        if ($this->login_user->user_type === 'client') {
            $view_data = $this->_get_project_info_data($project_id);
    ...
    public function project_member_modal_form()
    {
        $this->access_only_admin();
    ...
    public function save_project_member()
    {
        $this->access_only_admin();
        $data = array(
    ...
    public function delete_project_member()
    {
        $this->access_only_admin();
        $id = $this->input->post('id');
        if ($this->input->post('undo')) {
    ...
    public function project_member_list_data($project_id = 0)
    {
        $this->access_only_team_members();
    ...
    public function timer($project_id, $timer_status = 'start')
    {
        $this->access_only_team_members();
        $data = array(
    ...
    public function timelog_modal_form()
    {
        $this->access_only_team_members();
    ...
    public function timesheet_list_data($project_id = 0)
    {
        $this->access_only_team_members();
    ...
    public function milestone_modal_form()
    {
        $this->access_only_admin();
    ...
    public function milestones_list_data($project_id = 0)
    {
        $options = array('project_id' => $project_id);
    ...
    private function _make_milestone_row($data)
    {
        ...
        } elseif ($progress !== 100 && get_my_local_time('Y-m-d') > $data->due_date) {
    ...
    public function save_task()
    {
        ...
        foreach ($this->input->post('assigned_to') as $key => $value) {
            $dt = array(
                        'assigned_user' => $value,
                        'project_assigned' => $this->input->post('project_id'),
                        'task_assigned' => $save_id,
                        );
            $this->Crud_model->use_table('assigned_tasks')->save($dt);
        }
        if ($save_id) {
    ...
    public function tasks_list_data($project_id = 0)
    {
        $this->access_only_team_members();
    ...
    public function my_tasks_list_data()
    {
        $this->access_only_team_members();
        $status = $this->input->post('status') ? implode(',', $this->input->post('status')) : '';
        $project_id = $this->input->post('project_id');
    ...
    private function _make_task_row($data)
    {
        ...
        foreach (unserialize($data->assigned_to) as $key => $value) {
            foreach ($this->Users_model->get_details(['id' => $value])->result() as$user) {
                $image_url = get_avatar($user->image);
                $assigned_to_user = "<span class='avatar avatar-xs mr10'><img src='$image_url' alt='...'></span> $user->first_name $user->last_name";
                $assigned_to[] = get_team_member_profile_link($value, $assigned_to_user);
            }
        }
        ...
        if ($data->deadline) {
            ...
            } elseif (get_my_local_time('Y-m-d') == $data->deadline && $data->status != 'done') {
    ...
    public function customer_feedback($project_id)
    {
    ...
    public function view_comment_replies($comment_id)
    {
    ...
    public function comment_reply_form($comment_id, $type_id = 0, $type = 'project')
    {
    ...
    public function validate_project_file()
    {
    ...
    public function files_list_data($project_id = 0)
    {
        $this->access_only_team_members();
        $options = array('project_id' => $project_id);
    ...
    public function history($offset = 0, $project_id = 0)
    {
        $this->access_only_team_members();
        $view_data['project_id'] = $project_id;
        $view_data['offset'] = $offset;
    ...
    public function members($project_id = 0)
    {
        $this->access_only_team_members();
        $view_data['project_id'] = $project_id;
    ...
    public function bugs($project_id)
    {
        ...
        $this->load->view('projects/bugs/index', $view_data);
    }
}
